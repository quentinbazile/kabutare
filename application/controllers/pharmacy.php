...
class pharmacy extends CI_Controller
{
    ...
    public function fetch()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $captopril_begin = $this->input->post('captopril_begin');
            $captopril_received = $this->input->post('captopril_received');
            $captopril_dispensed = $this->input->post('captopril_dispensed');
            $captopril_expired = $this->input->post('captopril_expired');
            $captopril_end = $this->input->post('captopril_end');
            $captopril_stock = $this->input->post('captopril_stock');
            ...
            $this->pharmacy_model->new_report();

            $num_rapport = $this->pharmacy_model->num_rapport()->num_rapport;

            $this->pharmacy_model->fetch_data1(
              $amoxycillin_begin,
            ...
            $this->load->view('pharmacy_add_view');
        }
    }
}
